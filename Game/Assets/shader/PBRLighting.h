...
float CookTorranceSpecular(float3 L, float3 V, float3 N, float smooth) {
  ...
  // G項を求める
  float G =
      min(1.0f, min(2 * NdotH * NdotV / VdotH, 2 * NdotH * NdotL / VdotH));
...
// 反射光を計算する。
float3 CalcLighting(float3 ligDir, float3 ligColor, float3 normal, float3 toEye,
                    float4 albedoColor, float metaric, float smooth,
                    float3 specColor) {
  // 影が落ちていないのでライトの計算を行う。
  // ディズニーベースの拡散反射を実装する
  // フレネル反射を考慮した拡散反射を計算
  float diffuseFromFresnel =
      CalcDiffuseFromFresnel(normal, -ligDir, toEye, smooth);
  ...
  float3 spec = CookTorranceSpecular(-ligDir, toEye, normal, smooth) * ligColor;
  ...
  return max(float3(0.0f, 0.0f, 0.0f),
             diffuse * (1.0f - smooth) + spec * smooth);
}
